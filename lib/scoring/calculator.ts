...
export type MeasurementData = BalanceMeasurement | BreathMeasurement | GripMeasurement | HealthMeasurement;
 ...
 */
export async function getParticipantDemographics(participantId: string): Promise<ParticipantDemographics | null> {
...
export function calculateMeasurementScore(
  ...
  if (thresholds.average_score_min === null || thresholds.average_score_max === null) {
    return 1;
  }
 ...
 */
export function extractMeasurementValue(stationType: StationType, measurements: MeasurementData): { value: number; metricName: string } | null {
  switch (stationType) {
    case 'balance':
...
export async function calculateStationScore(
  ...
  try {
    ...
    if (!demographics) {
      console.warn(`Could not get demographics for participant ${participantId}, using default score`);
      return 1;
    }
    ...
    if (!measurementData) {
      console.warn(`Could not extract measurement for station type ${stationType}, using default score`);
      return 1;
    }
    ...
    if (!thresholds) {
      console.warn(`No scoring thresholds found for ${stationType}/${measurementData.metricName}/${demographics.gender}/${demographics.age_group}, using default score`);
      return 1;
    }
    ...
    console.log(`Calculated score for participant ${participantId}: ${stationType} = ${score} (${measurementData.value} ${measurementData.metricName})`);
