...
function DeepLinkProviderContent({ children }: DeepLinkProviderProps) {
  ...
  const [authFlow, setAuthFlowState] = useState<
  ...
  useEffect(() => {
    ...
    const storedRedirect =
      urlRedirect || localStorage.getItem(STORAGE_KEYS.REDIRECT_URL);
    const storedCompetition =
      urlCompetition || localStorage.getItem(STORAGE_KEYS.COMPETITION_ID);
    const storedFlow =
      urlFlow ||
      (localStorage.getItem(STORAGE_KEYS.AUTH_FLOW) as typeof authFlow);
    const storedOAuthState =
      urlOAuthState || localStorage.getItem(STORAGE_KEYS.OAUTH_STATE);
  ...
  const buildCompetitionEntryUrl = useCallback(
    (competitionId: string, requireAuth = true) => {
